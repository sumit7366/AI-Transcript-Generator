...
A modern Next.js-based tool for AI-powered YouTube video summarization. This application allows you to generate concise summaries of YouTube videos using different AI models, with support for multiple languages and summary styles also check out previous summaries topic and download in .txt file .
...
- **Multilingual Support**:
  - Generate summaries in National and International Language .
  - Clean formatting in all languages .
  - Proper handling of language-specific structures .
- **Flexible Summary Modes**:
  - Video Summary: Concise, structured overview
  - Podcast Style: More narrative, detailed analysis and also like two person communication structured.
- **Signup and LogIn** :
  - logIn : When click the generate button then give pop .
  - SignUp : if user not created Account then use to signUp .
- **Downlaod Summaries Text** :
  - Generated text download .
  - Previous topic download text through history .
- **Smart History System**:
  ...
  - also feature of download previous topic summaries text in the form of .txt file .
- **Modern UI/UX**:
...
### 3. LogIn Page for premium feature 
![Login Page](6.png)
user can also use premium feature after the login and store all history for future use and also inside that account story all history which will user used .
...
5. login and signup or skip the log in 
6. Watch the real-time progress
7. View your formatted summary
8. Download summeries Text in .txt file .
9. Access previous summaries in the history section
  - see all history  
  - Download previous Summaries text .
...
If you encounter database errors like "database disk image is malformed","Or facing something in Database " follow these steps:
...
- 📧 [Gmail](mailto:sumitranjanhisu@gmail.com)
...
- GPT-4o-mini
### API Key Flexibility:
- Use just one key to get started
- Interface adapts based on provided keys
### Language Support:
- National & international language summaries
- Clean formatting and language-specific structure handling
### Summary Modes:
Video Summary: Concise structured overview
Podcast Style: Conversational, narrative form
### Authentication System:
SignUp and LogIn
Pop-up triggered on summary generation if not logged in
### Smart History:
Stores all past summaries (SQLite)
View, manage, and download previous summaries
Ensures no duplicate video/language entries
Download the text in form of .txt file :
Export both current and historical summaries
### Modern UI/UX:
Tailwind CSS with responsive layout
Dark/light mode auto-toggle
Live summary generation progress
Markdown rendering
